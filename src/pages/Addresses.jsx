...
import { House, MapPin, Plus, Trash, PencilSimple, ArrowLeft, Crosshair } from "@phosphor-icons/react";
...
export default function Addresses() {
  ...
  const submit = async (e) => {
    ...
    setDraft(empty);
    setEditingId(null);
    // Force a refetch by toggling user dep (useAddresses re-runs on auth change).
    ...
    return (
      <div style={{ minHeight: "60vh", display: "grid", placeItems: "center", padding: 24 }}>
        <div style={{ textAlign: "center", maxWidth: 420 }}>
          <MapPin size={36} style={{ color: "var(--gold-dark)" }} />
          <h1 className="display-dt" style={{ fontSize: 28, marginTop: 14 }}>Sign in to manage addresses</h1>
          <p className="muted-dt" style={{ fontSize: 13, marginTop: 8 }}>
            Saved addresses are linked to your account so they show up at checkout and in your booking confirmations.
          </p>
          <Link to="/auth/login" className="btn-gold-dt" style={{ marginTop: 18 }}>Sign in</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <section className="site-section">
      <div className="container-dt max-w-[900px]">
        <Link to="/dashboard" className="inline-flex items-center gap-2 text-xs muted-dt mb-4">
          <ArrowLeft size={14} /> Back to dashboard
        </Link>
        <div className="eyebrow">Account</div>
        <h1 className="display-dt mt-2 text-5xl">Saved addresses</h1>
        <p className="mt-3 text-sm muted-dt max-w-xl">
          Add up to {MAX_ADDRESSES} delivery addresses. We use these for prasad delivery and ritual coordination.
        </p>

        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_.8fr]">
          {/* Form */}
          <form onSubmit={submit} className="panel-dt p-6">
            <div className="flex items-center justify-between">
              <h2 className="display-dt text-2xl">{editingId ? "Edit address" : "Add address"}</h2>
              {editingId && (
            ...
            <div className="mt-4 grid gap-3">
              <div className="grid grid-cols-2 gap-3">
                <label className="grid gap-1">
                  ...
                  <select
                    ...
                    className="h-10 rounded-xl border border-dt bg-transparent px-3 text-sm"
                  >
              ...
              <label className="grid gap-1">
                ...
                <textarea
                  ...
                  className="rounded-xl border border-dt bg-transparent px-3 py-2 text-sm"
                />
              </label>
              ...
              <div key={a.id} className="panel-dt p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => onEdit(a)} className="p-1.5 rounded hover:bg-black/5">
                      <PencilSimple size={14} />
                    </button>
                    <button onClick={() => onDelete(a.id)} className="p-1.5 rounded hover:bg-black/5" style={{ color: "#b3261e" }}>
                      <Trash size={14} />
                    </button>
                  </div>
                </div>
