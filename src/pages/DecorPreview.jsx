...
const ITEMS = [
  ...
  { name: "LeafBranch (flipped)", Comp: LeafBranch, desc: "Mirrored variant for symmetrical section corners.", code: '<LeafBranch flip />', props: { flip: true } },
  { name: "LotusLine", Comp: LotusLine, desc: "Stem-rising lotus with water line — hero & section accent.", code: "<LotusLine />" },
  { name: "MandalaRings", Comp: MandalaRings, desc: "Concentric rings + 16-ray + 8-petal lotus — the core motif.", code: "<MandalaRings />" },
  { name: "TempleArch", Comp: TempleArch, desc: "South-Indian gopuram arch with tiered roof lines.", code: "<TempleArch />" },
  { name: "DiyaLamp", Comp: DiyaLamp, desc: "Hanging oil lamp with flame, bowl & stand.", code: "<DiyaLamp />" },
  { name: "Kalash", Comp: Kalash, desc: "Sacred pot with coconut, mango leaves & base.", code: "<Kalash />" },
  { name: "DiyaCluster", Comp: DiyaCluster, desc: "Ring of 7 diya flames around a centre — Diwali pattern.", code: "<DiyaCluster />" },
  { name: "Swastika", Comp: Swastika, desc: "Auspicious right-facing swastika — stroke line-art.", code: "<Swastika />" },
  { name: "OmSymbol", Comp: OmSymbol, desc: "Sacred Om (Aum) with chandra + bindu.", code: "<OmSymbol />" },
  { name: "Trishul", Comp: Trishul, desc: "Shiva's three-pronged trident with shaft bands.", code: "<Trishul />" },
  { name: "Conch", Comp: Conch, desc: "Shankh sacred shell with spiral rings.", code: "<Conch />" },
  { name: "Rangoli", Comp: Rangoli, desc: "Symmetrical festival floor mandala — 8 + 8 petals.", code: "<Rangoli />" },
  { name: "SacredTree", Comp: SacredTree, desc: "Kalpavriksha wish-fulfilling tree with canopy fruits.", code: "<SacredTree />" },
  { name: "Hamsa", Comp: Hamsa, desc: "Sacred swan — vehicle of Saraswati, with water line.", code: "<Hamsa />" },
  { name: "Bell", Comp: Bell, desc: "Ghanta ritual bell with crown, rim & clapper.", code: "<Bell />" },
  { name: "Yantra", Comp: Yantra, desc: "Sri-yantra inspired geometric diagram — interlocking triangles.", code: "<Yantra />" },
  { name: "Toran", Comp: Toran, desc: "Mango-leaf door garland with catenary string.", code: "<Toran />" },
  { name: "Peacock", Comp: Peacock, desc: "Fanned-tail peacock — Karthikeya / Krishna motif.", code: "<Peacock />" },
];
      ...
      <section className="ink-dt relative overflow-hidden">
        ...
        <div className="container-dt relative py-10 sm:py-14">
          ...
          <div className="mt-6 flex flex-wrap items-start justify-between gap-6">
            <div>
              ...
              <p className="mt-4 max-w-xl text-[13px] leading-6 text-white/60">
                All stroke motifs from <code className="text-gold-200">Decor.jsx</code> rendered live. Use this page to
                pick the right ornament for a section, card or divider. Copy the JSX and drop it in.
              </p>
              <div className="mt-4 flex flex-wrap gap-2 text-[11px]">
                ...
                <span className="rounded-full bg-gold-400 text-ink-950 px-3 py-1.5 font-bold">{ITEMS.length} motifs + SacredMedallion</span>
              </div>
            </div>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <span className="text-[10px] font-bold tracking-[.14em] uppercase text-white/50">Preview tone</span>
            {[
              ["Gold", "gold"],
              ["Ink", "ink"],
              ["Muted", "ivory"],
              ...
              <button
                ...
                className={`rounded-full px-4 py-1.5 text-xs font-bold border transition ${
                  tone === val ? "bg-gold-400 text-ink-950 border-gold-400" : "bg-white/10 text-white border-white/15 hover:bg-white/15"
                }`}
              >
                {label}
              </button>
            ))}
            <span className="text-[11px] text-white/40">controls stroke color for the grid below</span>
          </div>
        </div>
      </section>
      ...
      <section className="site-section has-decor-dt relative">
        <div className="container-dt">
          <div className="flex items-end justify-between gap-4 flex-wrap">
            ...
            <span className="text-[11px] tracking-[.12em] uppercase font-bold text-[var(--muted)]">Gold layered disc · glow + rays · rope rim</span>
          </div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            <div className="rounded-[20px] border border-dt bg-[var(--surface)] p-6 flex flex-col items-center text-center">
              ...
              <code className="mt-2 text-[10px] font-mono bg-ink-950 text-gold-200 px-3 py-1 rounded-full">{"<SacredMedallion size={140} />"}</code>
            ...
            <div className="rounded-[20px] border border-dt bg-[var(--surface)] p-6 flex flex-col items-center text-center">
              ...
              <code className="mt-2 text-[10px] font-mono bg-ink-950 text-gold-200 px-3 py-1 rounded-full">{"<SacredMedallion size={96} glow={false} />"}</code>
            ...
            <div className="rounded-[20px] border border-dt bg-[var(--surface)] p-6 flex flex-col items-center text-center">
              ...
              <code className="mt-2 text-[10px] font-mono bg-ink-950 text-gold-200 px-3 py-1 rounded-full">{"<SacredMedallion size={96} rays={false} />"}</code>
          ...
          <div className="mt-4 rounded-[16px] border border-amber-200 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-900 px-4 py-3 text-xs leading-5 text-amber-900 dark:text-amber-200">
            <strong>Tip:</strong> Multiple medallions on one page are safe — each generates unique gradient ids via <code>React.useId()</code>. Works in both light & dark themes.
          </div>
        </div>
      </section>
      ...
      <section className="site-section has-decor-dt border-t border-dt">
        <div className="container-dt">
          ...
          <p className="mt-2 text-sm text-[var(--muted)] max-w-2xl">
            Two helpers in the same file — <code>SpinDecor</code> (scroll-linked rotation, respects reduced-motion) and{" "}
            <code>DrawDecor</code> (fade-in reveal for stroke SVGs).
          </p>

          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <div className="rounded-[20px] border border-dt bg-[var(--surface)] p-6">
              <div className="text-[11px] font-bold tracking-[.14em] uppercase text-gold-600">SpinDecor</div>
              ...
              <div className="mt-2 text-[11px] text-[var(--muted)]">Scroll the page — the mandala rotates with scroll. Pass reverse to invert.</div>
            </div>

            <div className="rounded-[20px] border border-dt bg-[var(--surface)] p-6">
              <div className="text-[11px] font-bold tracking-[.14em] uppercase text-gold-600">DrawDecor</div>
              <div className={`mt-4 flex justify-center ${toneMap[tone]}`}>
                <DrawDecor>
