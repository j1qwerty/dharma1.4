        ...
        <div className="container-dt">
          <div className="grid gap-3 lg:grid-cols-[1fr_auto]">
            ...
            <button className="btn-ghost-dt">
              {t("stories.featured")} <ArrowUpRight size={14} />
            </button>
          </div>
          <div className="mt-5 flex gap-2 overflow-x-auto pb-1">
            {cats.map((c) => {
              const count =
                c === "All" ? stories.length : stories.filter((s) => s.category === c).length;
              return (
                <button
                  onClick={() => setCat(c)}
                  key={c}
            ...
            <div className="grid gap-6 lg:grid-cols-[1.25fr_.75fr]">
                ...
                <Link to={`/stories/${feature.id}`} className="card-dt block">
                  <div className="media-dt aspect-[16/9]">
                    ...
                    <div className="absolute left-5 right-5 bottom-5 text-white">
                      ...
                      <h2 className="mt-2 display-dt text-4xl sm:text-5xl">{featureTitle}</h2>
              ...
              <Reveal>
                <div className="panel-dt h-full p-6 sm:p-7">
                  <div className="eyebrow">{t("stories.thisMonth")}</div>
                  <h2 className="display-dt mt-3 text-4xl">{t("stories.startRitual")}</h2>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-1 text-sm font-semibold leading-5">{sTitle}</div>
                          </div>
          ...
          <section className="mt-20 border-t border-dt pt-12 has-decor-dt">
            ...
            <div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr] items-end">
              <Reveal>
                <div className="eyebrow">{t("stories.keepReading")}</div>
                <h2 className="display-dt mt-3 text-5xl">{t("stories.livingArchive")}</h2>
              </Reveal>
              <Reveal>
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="panel-dt p-6">
                    <div className="text-3xl display-dt">{t("stories.festivalGuides")}</div>
                    <p className="mt-2 text-xs leading-6 text-muted-dt">
                      {t("stories.festivalGuidesCopy")}
                    </p>
                  </div>
                  <div className="panel-dt p-6">
                    <div className="text-3xl display-dt">{t("stories.templeHistories")}</div>
                    <p className="mt-2 text-xs leading-6 text-muted-dt">
                      {t("stories.templeHistoriesCopy")}
                    </p>
                  </div>
                </div>
              </Reveal>
